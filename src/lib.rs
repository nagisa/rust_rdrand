...
//!
//! `rdrand` is claimed to be a cryptographically secure PRNG. It is much
//! faster than `OsRng` (and slower than `StdRng`), but is only supported on
//! more recent Intel processors.
//!
//! The generator provided by this crate is a viable replacement to any
//! [std::rand](http://doc.rust-lang.org/std/rand/index.html) generator,
//! however, since nobody has audited Intel hardware yet, the usual disclaimers
//! apply.
...
impl RdRand {
    /// Build a generator object. The function will only succeed if `rdrand`
    /// instruction can be successfully used.
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
